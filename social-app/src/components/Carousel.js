  ...
  return (
    <>
      <div className={styles.imgContainer}>
        ...
        />
        <Button text="next" position={styles.next} onClick={handleNextImage} />
      </div>
    </>
  )
}

const Button = ({ text, position, onClick }) => {
  return (
    <button onClick={onClick} className={`${styles.buttons} ${position}`}>
      {text}
    </button>
  )
}
